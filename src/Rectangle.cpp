...
Rectangle::Rectangle(const uint32_t &centerX, const uint32_t &centerY,
...
void Rectangle::Remove() {
  ...
  for (int positonX = rect->centerX - rect->width;
       positonX <= rect->centerX + rect->width; positonX++) {
    for (int positonY = rect->centerY - rect->height / 2;
         positonY <= rect->centerY + rect->height / 2; positonY++) {
      WindowApi::gotoxy(positonX, positonY);
      if (positonY == rect->centerY - rect->height / 2 ||
          positonY == rect->centerY + rect->height / 2) {
        std::cout << " ";
      } else if (positonX == rect->centerX - rect->width ||
                 positonX == rect->centerX + rect->width) {
        std::cout << " ";
      } else {
        std::cout << " ";
      }
  ...
  Shape *rec =
      new Rectangle(rect->centerX, rect->centerY, rect->width, rect->height);
  ...
  for (int positonX = rect->centerX - rect->width;
    ...
    for (int positonY = rect->centerY - rect->height / 2;
      ...
      if (positonY == rect->centerY - rect->height / 2) {
        ...
        if (positonX == rect->centerX - rect->width ||
            positonX == rect->centerX + rect->width) {
